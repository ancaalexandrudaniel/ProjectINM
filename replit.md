...
7. **Structured Legal Articles Import System (Dec 2025)** - Complete system for importing legal articles with rich segment structure from LLM sessions. Format supports 7 segment types: official (text oficial), trad (traducere pe înțelesul studentului), puncte (puncte-cheie și capcane grilă), juris (jurisprudență relevantă), radar (ce apare la examen), logica (explicație de bază), conex (conexiuni cu alte articole). Database: legal_articles, legal_article_batches, legal_article_chunks tables. Located at `/legal-articles-import` page. Features: JSON validation, preview with segment badges, batch management, RAG processing endpoint (POST /api/legal-articles/:id/process-rag). Integrated with sidebar under "Bază de Date Juridică" section.
